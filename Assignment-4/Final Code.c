...
static int shell_dump(int argc, char *argv[])
{
	...
	while(j<=end_page)
	{
		...
		// for(i=0;i<64;i=i+8)
		// {
		// 	time_stamp_eerpom = ((uint32_t)read_buf1[i+0]<<24)|((uint32_t)read_buf1[i+1]<<16)|((uint32_t)read_buf1[i+2]<<8)|((uint32_t)read_buf1[i+3]);
		// 	printk("Time_stamp [page%d, sample%d : %d\n",j,p,time_stamp_eerpom);
		// 	dist_from_eeprom = ((uint32_t)read_buf1[i+4]<<24)|((uint32_t)read_buf1[i+5]<<16)|((uint32_t)read_buf1[i+6]<<8)|((uint32_t)read_buf1[i+7]);
